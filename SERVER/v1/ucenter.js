...
r.post('/changePassword',(req,res) =>{
    ...
    pool.objPool.query(sql,[user.upwd,user.uid],(err,rs) =>{
        if(err) throw err;
        res.send({rs});
    });
});
...
// 我发布的文章
r.get('/myArticle',(req,res) =>{
    let obj=req.query;
    console.log(obj);
	let sql='select aid,title,subtitle,img,created_at,view,comment from article  where author_id=? order by created_at asc';
    pool.objPool.query(sql,[obj.uid],(err,result)=>{
		if(err)throw err;
		res.send(result);
	})
});
...
// 我收藏的文章
r.get('/myFavorite',(req,res) =>{
    let obj=req.query;
    console.log(obj);
	let sql='select aid,title,subtitle,img,created_at,view,comment from article inner join favorite on aid=article_id where user_id=?';
    pool.objPool.query(sql,[obj.uid],(err,result)=>{
		if(err)throw err;
		res.send(result);
	})
...
// 我的评论列表
r.get('/myComment',(req,res) =>{
    let obj=req.query;
    console.log(obj);
	let sql='select aid,title,created_at,comment_content from article inner join article_comment on aid=article_id where user_id=?';
    pool.objPool.query(sql,[obj.uid],(err,result)=>{
		if(err)throw err;
		res.send(result);
	})
